...
def generate_navigator_reports(filename):
    ...
    with open('01_Model_Summary.md', 'w') as f:
        f.write("\n".join(summary_lines))
    ...
    with open('02_Model_Flowchart.md', 'w') as f:
        f.write("\n".join(flow_lines))
    ...
    with open('03_Calculation_Reference.md', 'w') as f:
        f.write("\n".join(calc_lines))
...
if __name__ == "__main__":
    ...
    if target_file:
        print(f"Generating Navigator reports for: {target_file}")
        generate_navigator_reports(target_file)
